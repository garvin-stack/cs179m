...
def parse_container_line(line):
    ...
    weight = int(weight[1:-1])  #Remove curly braces, convert weight to int
...
class ShipDisplayApp(QMainWindow):
    def __init__(self):
        ...
        self.layout.addWidget(self.move_info_label, 2,2)
    ...
    def get_containers_from_layout(self):
    ...
    def run_balance(self):
        ...
        self.canvas.figure = display_ship(read_ship_manifest(selected_file_path))
        self.canvas.draw()
    ...
    def show_next_step(self):
        ...
        self.canvas.figure = display_ship(self.get_containers_from_layout())
        self.canvas.draw()    
    ...
    def save_outbound_manifest(self):
        ...
        with open(outbound_file_path, "w") as outbound_file:
            for i in range(rows - 1):  # Exclude the 9th level
                for j in range(cols):
        ...
        # Display the reminder popup only once
        if not hasattr(self, 'reminder_popup_displayed'):
            self.show_reminder_popup()
            self.reminder_popup_displayed = True
